...
class DeletePatient extends React.Component {
    ...
    deletePatient = event => {
        ...
        this.props.client
           ...
           .then(res => {
               console.log(res)
               this.setState({ message: 'Patient Deleted.'})
           })
           .catch(err => {
               console.log(err);
           });
    ...
    render() {
        return (
        <form className="Patient-edit" onSubmit={this.deletePatient}>
        <div className="PatientInput">
        <Button intent="danger" iconBefore="trash" type="submit" onClick={() => toaster.success('Patient ' + this.state.firstName + ' ' + this.state.lastName + ' has been deleted.')
      }
      >Delete</Button>
        </div>
        <style jsx>{`
...
export default withApollo(DeletePatient);