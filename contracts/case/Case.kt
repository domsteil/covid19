...
data class Case(val caseId: UUID,
    ...
    override val participants = listOf(submitter, processor)
...
enum class CaseStatus {
    NEW, UNSTARTED, STARTED, WORKING, ESCALATED, CLOSED, OUTOFSCOPE, RESOLVED, RECOVERED
}
...
class CaseContract : Contract {
    ...
    private fun verifyCreate(tx: LedgerTransaction, signers: Set<PublicKey>) = requireThat {
                ...
                "the submitter should be different to the processor" using (output.processor != output.submitter)
    ...
    private fun verifyEscalate(tx: LedgerTransaction, signers: Set<PublicKey>) = requireThat {
                ...
                //      "Resolver should be a signer" using (caseCommand.signers.contains(caseOutput.processor.owningKey))
                        "one input should be produced" using (tx.inputStates.size == 1)
                        "one output should be produced" using (tx.outputStates.size == 1)
                        val output = tx.outputsOfType<Case>().single()
                        "the submitter should be different to the processor" using (output.processor != output.submitter)
